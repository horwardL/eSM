...
def get_cart_items(request):
    cart = get_cart(request)
    if cart:
        return CartItem.objects.filter(cart_unique_cart_id=_unique_cart_id(request))
...
def get_cart_total(request):
    ...
    if cart:
        cart_items = CartItem.objects.filter(cart_unique_cart_id=_unique_cart_id(request))
